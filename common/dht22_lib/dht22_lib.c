...
int readDHT1() {
    ...
    // Khởi tạo mảng dữ liệu DHT
    for (int k = 0; k < MAXdhtData; k++)
        dhtData[k] = 0;

    // Đặt chế độ OUTPUT cho chân GPIO
    gpio_set_direction(DHTgpio, GPIO_MODE_OUTPUT);
    gpio_set_level(DHTgpio, 0);
    esp_rom_delay_us(3000);  // Tạo xung thấp trong 3ms
    gpio_set_level(DHTgpio, 1);
    esp_rom_delay_us(25);    // Tạo xung cao trong 25us

    // Đặt chế độ INPUT cho chân GPIO
    gpio_set_direction(DHTgpio, GPIO_MODE_INPUT);

    // Đọc tín hiệu và kiểm tra thời gian phản hồi
    uSec = getSignalLevel(85, 0);
    if (uSec < 0) return DHT_TIMEOUT_ERROR;
    uSec = getSignalLevel(85, 1);
    if (uSec < 0) return DHT_TIMEOUT_ERROR;
    ...
    // Tính toán độ ẩm và nhiệt độ từ dữ liệu DHT
    humidity = dhtData[0];
    humidity *= 0x100;
    humidity += dhtData[1];
    humidity /= 10;
    ...
    // Kiểm tra tính hợp lệ của dữ liệu bằng cách so sánh checksum
    if (dhtData[4] ==
        ((dhtData[0] + dhtData[1] + dhtData[2] + dhtData[3]) & 0xFF))
        return DHT_OK;
    else
